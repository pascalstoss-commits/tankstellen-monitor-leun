...
def weekday_keys(dt):
    ...
    idx = dt.weekday()
    return names[idx], short[idx]


def text_matches_day(text, dt):
    if not text:
        return False
    t = text.strip().lower()
    full, short = weekday_keys(dt)
    if "mo-fr" in t or "montag-freitag" in t:
        return dt.weekday() <= 4
    if "sa-so" in t or "samstag-sonntag" in t:
        return dt.weekday() >= 5
    if "wochenende" in t:
        return dt.weekday() >= 5
    if "feiertag" in t and "sonntag" in t:
        return dt.weekday() == 6
    ...
    return any(k in t for k in mapping[dt.weekday()])
...
def compute_open_label(detail, now_local):
    ...
    times = detail.get("openingTimes") or []
    todays = [t for t in times if text_matches_day((t or {}).get("text", ""), now_local)]
    is_open = detail.get("isOpen")
    if is_open is True:
        ends = [format_hm(t.get("end")) for t in todays if t.get("end")]
        ends = [e for e in ends if e]
        if ends:
            return f"Geöffnet bis {ends[-1]}"
        return "Aktuell geöffnet"
    starts = [format_hm(t.get("start")) for t in todays if t.get("start")]
    starts = [s for s in starts if s]
    if starts:
        return f"Öffnet heute um {starts[0]}"
